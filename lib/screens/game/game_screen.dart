...
class _GameScreenState extends State<GameScreen> with TickerProviderStateMixin {
  ...
  double _worldW(BuildContext context) =>
      max(_mapW, MediaQuery.of(context).size.width);

  double _worldH(BuildContext context) =>
      max(_mapH, MediaQuery.of(context).size.height);

  @override
  void initState() {
    super.initState();

    // Force landscape when entering game
      ...
      bs.start(
        ...
        onTask: () {
          ...
          if (_bots[i].role == 'survivor' &&
            ...
            setState(() {
              _bots[i] = _bots[i]
                  .copyWith(tasksCompleted: _bots[i].tasksCompleted + 1);
            });
            _checkSoloWin();
          }
        },
  ...
  void _syncCamera() {
    ...
    final worldW = _worldW(context);
    final worldH = _worldH(context);

    final maxCamX = max(0.0, worldW - size.width);
    final maxCamY = max(0.0, worldH - size.height);

    _camX = (_myX - size.width / 2).clamp(0.0, maxCamX).toDouble();
    _camY = (_myY - size.height / 2).clamp(0.0, maxCamY).toDouble();
  }
  ...
  void _move(Timer _) {
    ...
    final double nx =
        (_myX + _jDx * speed).clamp(20.0, worldW - 20.0).toDouble();

    final double ny =
        (_myY + _jDy * speed).clamp(20.0, worldH - 20.0).toDouble();
  ...
  Future<void> _tickPortal() async {
    ...
    if (DateTime.now().isAfter(_portalDeadline!)) {
      ...
      final anySurvivor = _enteredPortalLocal ||
          _bots.any(
            (b) => b.role == 'survivor' && b.portalEntered && b.isAlive,
          );
  ...
  void _onRoom(RoomModel? room) {
    ...
    if (phaseChanged && _phase == 'portal') {
      _showMsg(
        '🌀 Portal opened! Enter within 2 minutes or die.',
        ToxicTheme.cyan,
      );
    }
  ...
  void _startRoundTwoSolo() {
    setState(() {
      ...
      final survivorsAlive = max(
        ...
        _bots
                .where(
                    (b) => b.role == 'survivor' && b.isAlive && b.portalEntered)
                .length +
  ...
  void _useAbility() {
    ...
    final double step = 1.0 / (td.abilityCooldownSec * 20.0);
    ...
    _cooldownTimer = Timer.periodic(const Duration(milliseconds: 50), (t) {
      ...
      setState(() {
        _abilityCooldown = (_abilityCooldown + step).clamp(0.0, 1.0).toDouble();
      });
    ...
    if (!widget.isSolo) {
      ...
      if (_tasks.every((t) => t.isCompleted)) {
        if (_round == 1) {
          ...
          _showMsg(
            '🌀 Portal opened! Enter within 2 minutes.',
            ToxicTheme.cyan,
          );
        } else {
          _goOver('survivors');
  ...
  void dispose() {
    ...
    // Return app back to portrait + landscape normal mode
    SystemChrome.setPreferredOrientations([
                ...
                child: Stack(
                  children: [
                    CustomPaint(
                      ...
                      painter:
                          _round == 1 ? ForestMapPainter() : NightmareMapPainter(),
                    ),
                    ...
                    ..._powerups.where((p) => !p.isCollected).map(
                          (p) => Positioned(
  ...
  Widget _buildHUD() {
    ...
    final int done = _tasks.where((t) => t.isCompleted).length;
    final int portalSecs = _phase == 'portal' && _portalDeadline != null
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Text(
                      'ROUND $_round',
                      style: TextStyle(
                        color: _round == 1 ? ToxicTheme.green : ToxicTheme.cyan,
                        fontSize: 10,
                        fontFamily: 'monospace',
                        fontWeight: FontWeight.bold,
                      ),
                    ...
                    children: [
                      SizedBox(
                        ...
                        child: CircularProgressIndicator(
                          ...
                          color: td.color,
                          backgroundColor: td.color.withOpacity(0.15),
                          strokeWidth: 3,
                        ),
                      ),
                      GestureDetector(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            border: Border.all(
                              color: _abilityReady
                                  ? td.color
                                  : td.color.withOpacity(0.35),
                              width: 2,
                            ),
                            boxShadow: _abilityReady
                                ? [
                                    BoxShadow(
                                      color: td.color.withOpacity(0.35),
                                      blurRadius: 10,
                                      spreadRadius: 1,
                                    )
                                  ]
                          ...
                          child: Icon(td.icon, color: td.color, size: 22),
                        ),
                      ),
                    ],
                  ),
                ...
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                      shape: BoxShape.circle,
                      color: _myRole == 'killer'
                          ? ToxicTheme.red.withOpacity(0.22)
                          : ToxicTheme.green.withOpacity(0.18),
                      border: Border.all(
                        color: _myRole == 'killer'
                            ? ToxicTheme.red
                            : ToxicTheme.green,
                    ...
                    child: Icon(
                      _myRole == 'killer' ? Icons.ads_click : Icons.task_alt,
                      color: _myRole == 'killer'
                          ? ToxicTheme.red
                          : ToxicTheme.green,
                      size: 34,
        ...
        child: Center(
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: ToxicTheme.greenDark, width: 1.3),
              boxShadow: [
                BoxShadow(
            ...
            child: Column(
              ...
              children: [
                ...
                ...List.generate(quiz.options.length, (i) {
                  final bool selected = _selectedAnswer == i;
                  final bool correct = i == quiz.correctIndex;

                  Color border = ToxicTheme.greenDark;
                  Color textColor = Colors.white;

                  if (_answerChecked) {
                    if (correct) {
                      border = ToxicTheme.green;
                      textColor = ToxicTheme.green;
                    } else if (selected && !correct) {
                      border = ToxicTheme.red;
                      textColor = ToxicTheme.red;
                    }
                  } else if (selected) {
                    border = ToxicTheme.cyan;
                    textColor = ToxicTheme.cyan;
                  }
